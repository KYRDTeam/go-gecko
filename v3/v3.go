...
func (c *Client) SimpleSinglePrice(ctx context.Context, id string, vsCurrency string, include24hrChange bool, includeLastUpdateAt bool) (*types.SimpleSinglePrice, error) {
	idParam := []string{strings.ToLower(id)}
	vcParam := []string{strings.ToLower(vsCurrency)}

	t, err := c.SimplePrice(ctx, idParam, vcParam, include24hrChange, includeLastUpdateAt)
	if err != nil {
		return nil, err
	}
	curr := (*t)[id]
...
func (c *Client) SimplePrice(ctx context.Context, ids []string, vsCurrencies []string, include24hChange, includeLastUpdateAt bool) (*map[string]map[string]float32, error) {
	params := url.Values{}
	...
	params.Add("include_24hr_change", strconv.FormatBool(include24hChange))
